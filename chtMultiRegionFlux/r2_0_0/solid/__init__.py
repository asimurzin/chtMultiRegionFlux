...
def solidRegionDiffNo( mesh, runTime, Cprho, K ):
    ...
    #  zero internal faces.
    KrhoCpbyDelta = mesh.deltaCoeffs() * ref.fvc.interpolate( K ) / ref.fvc.interpolate(Cprho)
...
def solidRegionDiffusionNo( solidRegions, runTime, thermos ):
    ...
    for index in range( solidRegions.__len__() ):
        mesh, thermo, rho, cp, K, T = setRegionSolidFields( index, solidRegions, thermos )
        DiNum = max( solidRegionDiffNo( solidRegions[ index ], runTime, rho * cp, K ), DiNum )
        pass
...
def setRegionSolidFields( i, solidRegions, thermos ):
    ...
    K = man.volScalarField( thermo.K(), man.Deps( thermo ) )
    # tmp<volSymmTensorField> tK = thermo.directionalK();
    
    # const volSymmTensorField& K = tK();
    T = man.volScalarField( thermo.T(), man.Deps( thermo ) )
    
    return mesh, thermo, rho, cp, K, T


#-------------------------------------------------------------------------------------------------------
def solveSolid( mesh, thermo, rho, cp, K, T, nNonOrthCorr, finalIter ):
    if finalIter:
        mesh.add( ref.word( "finalIteration" ), True )
        pass

    for index in range( nNonOrthCorr + 1 ):
       TEqn = ref.fvm.ddt( rho * cp, T ) - ref.fvm.laplacian( K, T )
       TEqn.relax()
